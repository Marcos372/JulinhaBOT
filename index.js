	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `SAIU MAIS UM CORNOKKKKKK @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				success: 'CONCLUIDO',
				error: {
					stick: 'Putz não consegui fazer sua figurinha',
					Iv: 'LINK INVÁLIDO'
				},
				only: {
					group: '❌ AVISO QUANDO ALGUEM USA UM COMANDO DE GRUPO NO PRIVADO❌',
					ownerG: 'APENAS ADMS DO GRUPO PODEM USAR',
					ownerB: '❌ SÓ PODE SER USADO PELO DONO DO BOT❌',
					admin: 'SÓ PODE SER USADO PELO DONO DO BOT❌',
					Badmin: 'TENHO QUE SER ADM'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["55994406484630@s.whatsapp.net"] // COLOQUE SEU NÚMERO AQUI SEM ESPAÇOS
...
//AQUI NAO PRECISA MEXER
	...
	//AQUI NAO PRECISA MUDAR
				...
				case 'ytsearch':
					if (args.length < 1) return reply(' O QUE A VC ESTA PROCURANDO ')
				...
				case 'nulis':
				case 'tulis':
					if (args.length < 1) return reply('MENSAGEM PERGUNTANDO O QUE VC QUER ESCREVE')
					teks = body.slice(7)
					reply(mess.wait)
			...
			case 'tagall':
				...
				case 'bc':
					...
					if (mentioned.length > 1) {
						teks = 'PARABÉNS ALEK VC FOI PROMOVIDO 😎👍\n'
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`REBAIXOU @${mentioned[0].split('@')[0]} REBAIXADO COM SUCESSO😪`, mentioned, true)
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply(' QUEM A VC QUER ADICIONAR') 
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('MARQUE QUEM VAI SER EXPULSO!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'SAIU DAQUI CORNO KSKSKSK:\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
